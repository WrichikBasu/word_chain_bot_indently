...
class CommonCog(Cog, name=COG_NAME_COMMON):
    ...
    async def _rejoin(self, config: ServerConfig, guild: discord.Guild, main_description: str) -> None:
        ...
        for game_mode in GameMode:
            try:
                channel = self.bot.get_channel(config.game_state[game_mode].channel_id)
    ...
    async def ensure_config(self, guild: discord.Guild, connection: AsyncConnection | None = None) -> None:
        ...
        if connection is None:
            async with self.bot.db_connection() as managed_connection:
    ...
    async def prepare_configs(self, /, *, perform_rejoin: bool):
        ...
        async with self.bot.db_connection() as connection:
            ...
            current_servers = {guild.id for guild in self.bot.guilds}
    ...
    async def add_word_to_cache(word: str, language: Language, connection: AsyncConnection) -> None:
        ...
        """
        if not await CommonCog.is_word_blacklisted(word):  # Do NOT insert globally blacklisted words into the cache
